...
class ChallengeManager {
    ...
    static async acceptChallenge(challengerUserId) {
        ...
        if (challenge) {
            ...
            console.log(`Challenge from ${challenge.challengerUserId} accepted by ${challenge.challengedUserId}.`);
            const challengerUserId = challenge.challengerUserId;
            const challengerProfile = await getProfileByUserId(challengerUserId);
            SocketSender.sendMessage(challengerUserId, "challengeAccepted", challengerProfile);
        } else {
    ...
    static async declineChallenge(challengerUserId) {
        ...
        if (challenge) {
            ...
            console.log(`Challenge from ${challenge.challengerUserId} declined by ${challengedUserId}.`);
            SocketSender.sendMessage(challengedUserId, "challengeDeclined");
        } else {
    ...
    static async cancelChallenge(challengerUserId) {
        ...
        if (challenge) {
            delete this.challenges[challenge.challengedUserId];
            const challengedUserId = challenge.challengedUserId;
            console.log(`Challenge to ${challengedUserId} cancelled by ${challengerUserId}.`);
            SocketSender.sendMessage(challengedUserId, "removeChallenge");
        } else {
            console.log(`LOG : ERROR: Cancelled non existing challenge ${challengerUserId}.`);
        }
    }

    static async addChallenge(challengerUserId, challengedUserId) {
        if (!this.challenges[challengedUserId]) {
            this.challenges[challengedUserId] = { challengerUserId, challengedUserId };
            console.log(`Challenge sent to user with ID ${challengedUserId} from user with ID ${challengerUserId}.`);
        } else {
            console.log(`User with ID ${challengedUserId} already has a pending challenge.`);
    ...
    static async sendChallenge(challengedUsername, challengerUserId) {
        ...
        SocketSender.sendMessage(challengedUserId, "receiveChallenge", challengerProfile);
    }

    static findChallengeByChallengerUserId(challengerUserId) {
        return Object.values(this.challenges).find(challenge => challenge.challengerUserId === challengerUserId);
    }
