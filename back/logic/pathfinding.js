function possiblePath(gameBoard, players) {
    let possible = false;
    for (var i = 0; i < 2; i++) {
        if (checkPath(gameBoard, players[i])) {
            possible = true;
        } else {
            return false;
        }
...
function checkPath(gameBoard, player) {
    var queue = [];
    var visited = [];
    ...
    while (queue.length > 0) {
        ...
        var x = current[1];
        var y = current[0];
        if ((y === 0 && player.playerNumber === 2) || (y === 16 && player.playerNumber === 1)) {
            return true;
        }
...
function testPathfinding() {
    ...
    console.log(possiblePath(gameBoard, [player1, player2]));
    for (let i = 0; i < 17; i++) {
        gameBoard[9][i] = 1;
    }
    console.log(possiblePath(gameBoard, [player1, player2]));
