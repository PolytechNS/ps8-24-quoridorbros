...
class AchievementsManager {
    ...
    try {
      ...
      if (currentProgression < achievement.out) {
        ...
        await userProfileCollection.updateOne(
          ...
          {
            $set: {
              [`achievements.${achievementId}.progression`]: updatedProgression,
            },
          },
        );
  ...
  static async reinitializeAchievement(
    userProfileCollection,
    userId,
    achievementId,
  ) {
      ...
      for (const achievementId in achievements) {
        if (!existingAchievements.hasOwnProperty(achievementId)) {
          updatedAchievements[achievementId] = {
            id: achievements[achievementId].id,
            description: achievements[achievementId].description,
            progression: 0,
            out: achievements[achievementId].out,
            notify: achievements[achievementId].notify,
          };
        }
      }
      await userProfileCollection.updateOne(
  ...
  static async getNotifiedAchievements(
    userProfileCollection,
    ...
    try {
      ...
      for (const achievementId in achievements) {
        ...
        if (
          achievement.progression === achievement.out &&
          !achievement.notify
        ) {
